...
static void* thread_fcn(void*ptr){
	...
	if (errorMessage){
		...
		ov_string_setvalue(&pthreadData->pDiscoveryServer->v_ErrorMessage, "Error in part-function");
	...
	if (resultOV){
		ov_string_setvalue(&pthreadData->pDiscoveryServer->v_ErrorMessage, "Error in sendMessage");
		pthreadData->pDiscoveryServer->v_ErrorFlag = TRUE;
...
OV_DLLFNCEXPORT OV_RESULT openAASDiscoveryServer_DiscoveryServer_getMessage(OV_INSTPTR_openAASDiscoveryServer_DiscoveryServer pinst, const OV_STRING JsonInput, OV_STRING *errorMessage) {
	...
	// TODO if JsonInput is NULL no need to go further?
	...
	switch (protocolType){
		case 1: { // MessageSys
			...
			if(Ov_Fail(resultOV)){
				...
				if(!pinst->v_ErrorFlag) {
					ov_string_setvalue(&pinst->v_ErrorMessage, "Could not create an answerMessage");
					pinst->v_ErrorFlag = TRUE;
				}
				return OV_ERR_GENERIC;
			}
